...
def build_core_signals(cfg: ConfigLike, feature_frame: FrameLike) -> FrameLike:
    ...
    required = [ma_fast_col, ma_slow_col, return_col, volatility_col]
    ...
    ma_spread_raw = (ma_fast - ma_slow) / frame["Close"].astype(float)
    ...
    frame["ma_spread_signal"] = _scaled_tanh(ma_spread_raw, scale=trend_scale)
